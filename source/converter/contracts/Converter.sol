...
contract Converter is Ownable, TokenPaymentSplitter {
     ...
     */
    function setTokenPath(address _token, address[] memory _tokenPath) public onlyOwner {
    ...
    function convertAndTransfer(
        ...
        for (uint256 i = 0; i < _payees.length; i++) {
            uint256 payeeAmount = (totalPayeeAmount.mul(_shares[_payees[i]])).div(_totalShares);
            _transferErc20(
                _payees[i],
                swapToToken,
                payeeAmount
     ...
     */
    function getTokenPath(address _token) public view onlyOwner returns(address[] memory) {
